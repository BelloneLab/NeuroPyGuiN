...
def _install_sklearn_pickle_aliases():
    import sklearn.ensemble._forest as _forest  # pylint: disable=import-error
    import sklearn.tree._classes as _tree_classes  # pylint: disable=import-error

    sys.modules.setdefault("sklearn.ensemble.forest", _forest)
    sys.modules.setdefault("sklearn.tree.tree", _tree_classes)


def _coerce_legacy_tree_nodes(node_ndarray, expected_dtype):
    node_dtype = getattr(node_ndarray, "dtype", None)
    if node_dtype is None or node_dtype.names is None or expected_dtype.names is None:
        return None
    expected_names = tuple(expected_dtype.names)
    legacy_names = tuple(node_dtype.names)
    if "missing_go_to_left" not in expected_names or "missing_go_to_left" in legacy_names:
        return None
    expected_legacy_names = tuple(name for name in expected_names if name != "missing_go_to_left")
    if legacy_names != expected_legacy_names:
        return None

    converted = np.empty(node_ndarray.shape, dtype=expected_dtype)
    for name in legacy_names:
        converted[name] = node_ndarray[name]
    converted["missing_go_to_left"] = 0
    return converted


def _backfill_estimator_defaults(estimator):
    try:
        parameters = inspect.signature(type(estimator)).parameters
    ...
    for name, param in parameters.items():
        ...
        if not hasattr(estimator, name):
            setattr(estimator, name, param.default)
    if hasattr(estimator, "n_features_") and not hasattr(estimator, "n_features_in_"):
        setattr(estimator, "n_features_in_", int(estimator.n_features_))
    return estimator


def _backfill_legacy_random_forest(classifier):
    _backfill_estimator_defaults(classifier)

    if hasattr(classifier, "estimators_"):
        for estimator in classifier.estimators_:
            _backfill_estimator_defaults(estimator)

    if not hasattr(classifier, "estimator"):
        base = getattr(classifier, "base_estimator", None) or getattr(classifier, "base_estimator_", None)
        if base is None:
            base = DecisionTreeClassifier(criterion=getattr(classifier, "criterion", "gini"))
        classifier.estimator = _backfill_estimator_defaults(base)
    if not hasattr(classifier, "estimator_"):
        classifier.estimator_ = classifier.estimator
    if not hasattr(classifier, "n_features_in_") and getattr(classifier, "estimators_", None):
        first_tree = classifier.estimators_[0]
        if hasattr(first_tree, "n_features_in_"):
            classifier.n_features_in_ = int(first_tree.n_features_in_)
    return classifier


@contextmanager
def _patched_tree_node_loader():
    import sklearn.tree._tree as _tree  # pylint: disable=import-error

    original = _tree._check_node_ndarray

    def _compat_check_node_ndarray(node_ndarray, expected_dtype):
        try:
            return original(node_ndarray, expected_dtype)
        except ValueError as exc:
            if "node array from the pickle has an incompatible dtype" not in str(exc):
                raise
            converted = _coerce_legacy_tree_nodes(node_ndarray, expected_dtype)
            if converted is None:
                raise
            return converted

    _tree._check_node_ndarray = _compat_check_node_ndarray
    try:
        yield
    finally:
...
def _load_classifier_compat(classifier_path):
    """
    Load legacy RF pickle files trained with older scikit-learn versions.
    Older pickles may reference module paths that moved in newer sklearn
    releases, such as `sklearn.ensemble.forest` and `sklearn.tree.tree`.
    """
    _install_sklearn_pickle_aliases()
    with _patched_tree_node_loader():
        with open(classifier_path, 'rb') as f:
            classifier = pickle.load(f)
    if isinstance(classifier, RandomForestClassifier):
        classifier = _backfill_legacy_random_forest(classifier)
    return classifier
...
def id_noise_templates_rf(spike_times, spike_clusters, cluster_ids, templates, params):
    ...
    classifier = _load_classifier_compat(classifier_path)
