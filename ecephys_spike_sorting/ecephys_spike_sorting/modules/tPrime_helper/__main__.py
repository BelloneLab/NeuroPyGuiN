...
def call_TPrime(args):
    ...
    if not all_fyi_exists:
        ...
        print('Checking for _fyi.txt file from CatGT run outside ecephys spike sorting pipeline.')
    ...
    proc = subprocess.run(tcmd, shell=True)
    if proc.returncode != 0:
        raise RuntimeError(f'TPrime exited with code {proc.returncode}')
    ...
    missing_outputs = [op for op in out_list if not Path(op).is_file()]
    if missing_outputs:
        raise FileNotFoundError(
            'TPrime finished without creating expected adjusted outputs: '
...
def _resolve_fyi_generated_path(curr_path, run_directory):
    raw_path = str(curr_path).strip()
    if not raw_path:
        return raw_path
    path = Path(raw_path)
    if path.is_file():
    ...
    if not run_dir.exists():
        return raw_path
    matches = sorted(
        run_dir.rglob(path.name),
        key=lambda p: ("imec" not in p.parent.name.lower(), len(str(p)), str(p).lower()),
    )
    if matches:
        resolved = matches[0].resolve().as_posix()
        if resolved != raw_path.replace("\\", "/"):
            print(f"Resolved FYI path: {raw_path} -> {resolved}")
        return resolved
    return raw_path
...
def parse_catgt_fyi(fyi_path, toStream_id):
    ...
    out_list = []
    all_list = []
    run_directory = Path(fyi_path).parent
    ...
    with open(fyi_path, 'r') as reader:
        ...
        while line != '':  # The EOF char is an empty string
            ...
            if line.find('sync') == 0:                
                ...
                curr_path = _resolve_fyi_generated_path(curr_path[0:len(curr_path)-1], run_directory)
                all_list.append(curr_path)
                js, ip = parse_stream(stream_str)
                if not Path(curr_path).is_file():
                    raise FileNotFoundError(f"Missing sync file referenced in FYI: {curr_path}")
                ...
                if (js,ip) != toStream_id:
                    if not Path(curr_path).is_file():
                        print(f"Warning: skipping missing TPrime event file: {curr_path}")
                        line = reader.readline()
                        continue
