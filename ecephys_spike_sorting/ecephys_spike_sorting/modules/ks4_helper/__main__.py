...
def _resolve_phy_dat_file(dat_path, dat_name):
    raw_path = dat_path
    try:
        raw_path = ast.literal_eval(str(dat_path))
    except Exception:
        pass
    if isinstance(raw_path, (list, tuple)):
        raw_path = raw_path[0] if raw_path else ""
    base_path = Path(str(raw_path)).expanduser() if str(raw_path).strip() else Path()

    raw_name = dat_name
    try:
        raw_name = ast.literal_eval(str(dat_name))
    except Exception:
        pass
...
def _fix_phy_params(output_dir, dat_path, dat_name, chan_phy_binary,
    ...
    new_path = str(_resolve_phy_dat_file(dat_path, dat_name)).replace('\\', '/')
...
def run_ks4(args):
    ...
    """
    Run full ecephys spike sorting pipeline on specified data.
     ...
     settings : dict
         ...
         ecephys spike sorting pipeline. See `kilosort/parameters.py` for a full list of
    ...
    #check if chanMap file already exists
    # if os.path.exists(chanmap_file):
    # import pdb
    # pdb.set_trace()
    ...
    print('helloooooo',repr(settings))
#    print(repr(ks4_prb))
