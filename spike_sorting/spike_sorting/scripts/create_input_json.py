            ...
            for m in mdatList:
                ...
                try:
                    if csList[0][0] == '~':
                        ...
                        currKey = csList[0]
                    metaDict.update({currKey: csList[0]})
                except:
                    pdb.set_trace()
                if len(csList) > 1:
                    metaDict.update({currKey: csList[1]})
                else:
                    metaDict.update({currKey: ''})
...
def createInputJson(output_file,
                    ...
                    external_catgt_path = None,
                    external_tprime_path = None,
                    external_cwaves_path = None,
                    external_ks4_repo_path = None,
                    external_kilosort_output_tmp = None,
                    external_npy_matlab_repository = None,
                    ):

    # Resolve bundled ecephys_spike_sorting package directory dynamically.
    ecephys_directory = str(Path(__file__).resolve().parents[1])

    # location of kilosort repositories for MATLAB versions.
    ...
    if ks_ver == '2.0':
        ...
        kilosort_repository = r'D:\NPX\preprocessing\Kilosort\kilosort'  # default path for when we aren't using any of these
    if external_ks4_repo_path not in [None, ""]:
        kilosort_repository = str(external_ks4_repo_path)

    npy_matlab_repository = r'D:\NPX\preprocessing\npy-matlab'
    if external_npy_matlab_repository not in [None, ""]:
        npy_matlab_repository = str(external_npy_matlab_repository)
    catGTPath = r'D:\NPX\preprocessing\CatGT-win'
    if external_catgt_path not in [None, ""]:
        catGTPath = str(external_catgt_path)
    tPrime_path=r'D:\NPX\preprocessing\TPrime-win'
    if external_tprime_path not in [None, ""]:
        tPrime_path = str(external_tprime_path)
    cWaves_path=r'D:\NPX\preprocessing\C_Waves-win'
    if external_cwaves_path not in [None, ""]:
        cWaves_path = str(external_cwaves_path)

    # for config files and kilosort working space
    kilosort_output_tmp = r'D:\kilosort_datatemp'
    if external_kilosort_output_tmp not in [None, ""]:
        kilosort_output_tmp = str(external_kilosort_output_tmp)
    ...
    if spikeGLX_data:
        ...
        if input_meta_path is not None:
            print('Reading SpikeGLX params from meta file: ' + input_meta_path)
            # pdb.set_trace()
            # metadict= readMeta(input_meta_path)

            probe_type, sample_rate, num_channels, reference_channels,uVPerBit, vpitch, hpitch, nColumn, nAP, nSY, useGeom= SpikeGLX_utils.EphysParams(input_meta_path)
