...
def _load_classifier_compat(classifier_path):
    ...
    Older pickles may reference `sklearn.ensemble.forest`, which moved to
    `sklearn.ensemble._forest` in newer sklearn releases.
    """
    with open(classifier_path, 'rb') as f:
        try:
            return pickle.load(f)
        except ModuleNotFoundError as exc:
            if "sklearn.ensemble.forest" not in str(exc):
                raise
    # Retry once with compatibility alias.
