...
def get_all_quality_metrics(
    ...
    for unit_idx in tqdm(range(unique_templates.size), bar_format=bar_description):
        ...
        time_tmp = time.time()
        pc_shape0 = getattr(pc_features, "shape", (0,))[0] if pc_features is not None else 0
        can_compute_drift = (
            param.get("computeDrift", True)
            and pc_features is not None
            and pc_features_idx is not None
            and pc_shape0 == these_spike_clusters.shape[0]
        )
        if can_compute_drift:
            (
            ...
            quality_metrics["maxDriftEstimate"][unit_idx] = np.nan
            quality_metrics["cumDriftEstimate"][unit_idx] = np.nan
    ...
    tau_r_path = Path(save_path) / "templates._bc_fractionRefractoryPeriodViolationsPerTauR"
    try:
        df.to_parquet(str(tau_r_path) + ".parquet")
    except Exception:
        # Parquet engine is optional; keep pipeline running with CSV fallback.
        df.to_csv(str(tau_r_path) + ".csv")
