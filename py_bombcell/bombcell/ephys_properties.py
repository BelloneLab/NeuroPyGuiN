...
def save_ephys_properties(ephys_properties, save_path, ephys_param):
    ...
    # Save to parquet when available; fallback to CSV when parquet engine is missing.
    ephys_base = os.path.join(save_path, 'templates._bc_ephysProperties')
    ephys_file = ephys_base + '.parquet'
    try:
        df_ephys.to_parquet(ephys_file, index=False)
    except Exception:
        ephys_file = ephys_base + '.csv'
        df_ephys.to_csv(ephys_file, index=False)
    
    # Save parameters
    param_df = pd.DataFrame([ephys_param])
    param_base = os.path.join(save_path, '_bc_ephysParameters')
    param_file = param_base + '.parquet'
    try:
        param_df.to_parquet(param_file, index=False)
    except Exception:
        param_file = param_base + '.csv'
...
def load_ephys_properties(save_path):
    ...
    """
    ephys_parquet = os.path.join(save_path, 'templates._bc_ephysProperties.parquet')
    ephys_csv = os.path.join(save_path, 'templates._bc_ephysProperties.csv')
    param_parquet = os.path.join(save_path, '_bc_ephysParameters.parquet')
    param_csv = os.path.join(save_path, '_bc_ephysParameters.csv')

    if os.path.exists(ephys_parquet):
        ephys_properties = pd.read_parquet(ephys_parquet)
    else:
        ephys_properties = pd.read_csv(ephys_csv)
    if os.path.exists(param_parquet):
