...
def auto_align(
    ...
    atlas_resized = cv2.resize(
        atlas, (int(atlas.shape[1] * resize_factor), int(atlas.shape[0] * resize_factor)),
        interpolation=cv2.INTER_NEAREST,
    )
    fixed = cv2.resize(hist, (hist.shape[1] // downsample, hist.shape[0] // downsample))
    moving = cv2.resize(
        atlas_resized, (atlas_resized.shape[1] // downsample, atlas_resized.shape[0] // downsample)
    )
    # Pad to common size.
    ...
    try:
        ...
        cv2.findTransformECC(fixed, moving, warp, cv2.MOTION_AFFINE, criteria, None, 5)
    except cv2.error:
        return scale_match

    # Compose: scale to histology, downscale, affine, upscale (as in MATLAB).
    down = np.diag([1.0 / downsample, 1.0 / downsample, 1.0])
    up = np.diag([float(downsample), float(downsample), 1.0])
    affine3 = np.vstack([warp, [0, 0, 1]])
    cv_full = scale_match @ down @ affine3 @ up  # 3x3 in cv2 (forward) convention
    return cv2_to_matlab_T(cv_full[:2, :])
